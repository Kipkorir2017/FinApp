...
import "../index.css";
import { FaPhoneAlt, FaArrowLeft, FaCheckCircle, FaTimesCircle, FaCopy } from "react-icons/fa";
...
function Borrowers() {
  ...
  // Direct API URLs for now
  const BORROWERS_API = "http://localhost:5000/api/borrowers";
  ...
  const fetchBorrowers = async () => {
    try {
      ...
      console.error(err);
      alert("Failed to fetch borrowers. Check your token.");
    }
  };
  ...
  const saveComment = async id => {
    try {
      await axios.put(
        `${BORROWERS_API}/${id}`,
        { notes: borrowerComments[id], status: borrowerStatus[id] || null },
        axiosConfig
      );
      alert("Comment saved!");
    } catch (err) {
            ...
            {borrowers.map(b => (
              <React.Fragment key={b._id}>
                <tr className={borrowerStatus[b._id] === "Called" ? "called-row" : ""}>
                  ...
                  <td>{new Date(b.dob).toLocaleDateString()}</td>
                  ...
                  <td className="status-column">
                    <FaCheckCircle
                      className={`status-button called-btn ${borrowerStatus[b._id] === "Called" ? "active" : ""}`}
                      onClick={() => markStatus(b._id, "Called")}
                      title="Mark as Called"
                    />
                    <FaTimesCircle
                      className={`status-button unreachable-btn ${borrowerStatus[b._id] === "Unreachable" ? "active" : ""}`}
